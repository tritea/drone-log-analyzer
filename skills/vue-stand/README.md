...
> 规范以项目**当前真实写法**为准：源码在 `frontend/src/`，Pinia **setup store**（`defineStore(name, () => {...})` + `reactive()`/`computed()`），后端调用经 `services/{log,map,config}` 的 client（不走 HTTP `/api`），主图是自绘 `LineChart`（无 ECharts），`<script setup>` + `storeToRefs`。完整内容见 [SKILL.md](./SKILL.md)。
...
6. **非响应式重对象** —— `LineChart` / Three.js / MapLibre / Leaflet 实例 + timer 句柄放 [`utils/runtime.ts`](../../frontend/src/utils/runtime.ts)，不进 Pinia state。
