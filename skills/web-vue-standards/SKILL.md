...
技术栈：Vue 3（`<script setup>` + Composition API）+ Pinia 3（**setup store**）+ Three.js r160 + MapLibre GL 5.24 + Leaflet 1.9，TypeScript，Vite 5 编译，Go embed。**主图是自绘 [`LineChart`](../../frontend/src/chart/line-chart.ts)，不用 ECharts。**
...
| `echarts.init(el)` | `new LineChart(el, ...)`（放 `runtime.mainChart`） | ECharts 已移除，主图自绘 |
...
`LineChart` 实例、Three.js 视图、MapLibre/Leaflet 地图、RAF/timer 句柄这些**不该进响应式系统**的重对象，统一放 [`utils/runtime.ts`](../../frontend/src/utils/runtime.ts) 的模块级容器：

```typescript
import { runtime } from '@/utils/runtime';

if (!runtime.mainChart) runtime.mainChart = new LineChart(el, dataset, options);
```

```typescript
export const runtime: {
  mainChart: LineChart | null;        // 自绘主图（不是 echarts）
...
| `echarts.init(el)` | `new LineChart(...)`（放 `runtime.mainChart`） |
...
| 把 LineChart/Three/MapLibre 实例塞进 Pinia state | 放 `runtime` |
...
5. **重对象？** LineChart/Three/Map → `runtime`。
