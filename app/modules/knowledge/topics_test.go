...
// TestTopicChainParamsCoverKB 校验排查链引用的参数都在参数知识库中（拼错
// 即失败），且 tlog 回退 apm、未知格式无链。
func TestTopicChainParamsCoverKB(t *testing.T) {
	pk := ForParams("apm")
	for topic, chain := range formatChains["apm"] {
		if len(chain.Params) == 0 || chain.Guide == "" {
			t.Errorf("apm 主题 %q 排查链不完整", topic)
		}
		for _, p := range chain.Params {
			if _, ok := pk.Lookup(p); !ok {
				t.Errorf("apm 主题 %q 排查链参数 %q 不在参数知识库", topic, p)
			}
		}
	}
	...
	if _, ok := TopicChainFor("ulog", "altitude"); ok {
		t.Error("ulog 排查链未充实，应返回无")
	}
	if _, ok := TopicChainFor("apm", "vibration"); ok {
		t.Error("未定义链的主题应返回无")
	}
}
