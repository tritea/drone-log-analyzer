...
// providerInfo describes p for listing UIs, marking non-imagery providers
// (terrain DEM etc.) as Hidden so basemap pickers can filter them out.
func providerInfo(p Provider) ProviderInfo {
	info := ProviderInfo{ID: p.ID(), Name: p.Name(), Attribution: p.Attribution()}
	if h, ok := p.(hideable); ok {
