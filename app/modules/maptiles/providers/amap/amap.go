...
// NewSatellite builds the AMap satellite tile provider.
func NewSatellite(storage *maptiles.MBTilesStorage, client *http.Client) maptiles.Provider {
	return maptiles.NewTileSource(
		"amap_satellite",
		"高德卫星地图",
		"© AutoNavi (高德)",
		...
		func(z, x, y int) string {
			n := hostIndex(x, y)
			return fmt.Sprintf("https://webst0%d.is.autonavi.com/appmaptile?style=6&x=%d&y=%d&z=%d", n, x, y, z)
		},
	)
}
