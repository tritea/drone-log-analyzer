...
「可视化」视图内的「地图」渲染项：前端 Leaflet（2D）/ Cesium（3D 地图）通过 Go 中转取瓦片，Go 侧按 provider 抽象（`esri_satellite` / `amap_vector` / `amap_road` / `bing_road` / `osm`），单一共享 MBTiles 缓存，缓存优先、未命中抓取上游并落盘。
...
- 瓦片路由保留 HTTP（Leaflet/Cesium 按 URL 加载 `/map/provider/{id}/{z}/{x}/{y}`），进程内同源（Wails AssetServer.Handler），无独立端口；其余地图接口经 `MapAPI` 绑定。
