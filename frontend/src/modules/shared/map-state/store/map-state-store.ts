...
export const useMapStateStore = defineStore('map-state', () => {
  ...
  async function loadProviders(): Promise<void> {
    ...
    try {
      ...
      const providers = (Array.isArray(payload.providers) ? payload.providers : []).filter((p) => !p.hidden);
      ...
      showToast(tr('map.cache.sourceLoadFailed') + ': ' + describeError(e), 'error');
    } finally {
  ...
  async function loadCacheStats(): Promise<void> {
    ...
    try {
      ...
      showToast(tr('map.cache.statsLoadFailed', { err: describeError(e) }), 'error');
    } finally {
  ...
  async function clearCacheProvider(id: string): Promise<void> {
    try {
      ...
      if (err) { showToast(err, 'error'); return; }
      showToast(tr('map.cache.cleaned'), 'info');
      await loadCacheStats();
    } catch (e: unknown) {
      showToast(tr('map.cache.cleanFailed', { err: describeError(e) }), 'error');
    }
  }
  ...
  async function clearCacheAll(): Promise<void> {
    try {
      ...
      if (err) { showToast(err, 'error'); return; }
      showToast(tr('map.cache.clearedAll'), 'info');
      await loadCacheStats();
    } catch (e: unknown) {
      showToast(tr('map.cache.cleanFailed', { err: describeError(e) }), 'error');
    }
