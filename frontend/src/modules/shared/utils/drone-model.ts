...
export interface BuiltDroneModel {
  group: THREE.Group
...
export function makeDroneQuaternionMercator(sample: AttitudeSample): THREE.Quaternion {
  const deg = Math.PI / 180
  const yaw = (sample.yaw || 0) * deg
  const pitch = (sample.pitch || 0) * deg
  const roll = (sample.roll || 0) * deg
...
export function buildDroneModel(
  ...
  opts: {
    ...
    airframe?: string
    propOrder?: Record<string, Array<{ name: string; dir: number; func?: number }>>
    materialTable?: Record<string, Record<string, MaterialProperties>>
    fallbackMaterial?: MaterialProperties
    buildMaterial: MaterialBuilder
  },
): BuiltDroneModel {
  const scene = opts.lowpoly ? buildLowpolyDroneModel(name) : opts.glbScene
  if (!scene) throw new Error('buildDroneModel: lowpoly=false 时须提供 glbScene')
  const group = normalizeLoadedDroneModel(
    scene,
    { x: 0.01, y: 0.01, z: 0.01 },
    { x: false, y: true, z: true },
    name,
    opts.materialTable ?? THREE_MODEL_MATERIAL,
    opts.fallbackMaterial ?? BODY_MATERIAL,
    opts.buildMaterial,
  )
  const propellers = collectPropellers(group, name, opts.propOrder ?? THREE_PROPELLER_ORDER)
  const phys = computeDronePhysicalScale(group, opts.airframe)
  const baseScaleMeters = phys ? phys.scale : 1
  return { group, propellers, baseScaleMeters }
}
