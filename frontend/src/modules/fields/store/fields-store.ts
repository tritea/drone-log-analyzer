...
export const useFieldsStore = defineStore('fields', () => {
  ...
  function readEntriesFromJson(text: string): FieldEntry[] {
    ...
    if (!Array.isArray(list)) throw new Error('配置文件必须包含 entries 数组');
    const entries = list
    ...
    if (!entries.length) throw new Error('没有可导入的组');
    return entries;
  }
  ...
  async function applyFieldEntry(entry: FieldEntry): Promise<void> {
    ...
    analysis.setFieldGroupParams(entry.name, entry);
    try {
      await analysis.addCurveSelection(entry.curves, entry.name);
    } finally {
  ...
  function simpleFieldGroupParams(entry: FieldEntry | null): FieldGroupParams {
    const analysis = useAnalysisStore();
    if (!entry || !entry.name) return analysis.makeFieldGroupParams(entry);
    return analysis.ensureFieldGroupParams(entry.name, entry);
  }
  ...
  function setActiveFieldVisible(name: string, event: Event): void {
    ...
    analysis.scheduleCurveStateSave();
  }
  ...
  function removeFieldCurves(name: string): void {
    ...
    analysis.rebuildChart();
    void analysis.saveCurveStateNow();
  }
  ...
  async function loadFields(): Promise<void> {
    ...
    try {
      ...
      showToast('加载组失败: ' + (error as Error).message, 'error');
    } finally {
  ...
  function currentFieldEntryPayload(
    ...
    const analysis = useAnalysisStore();
    const group = analysis.ensureFieldGroupParams(name);
  ...
  async function performDeleteFieldEntry(
    ...
    try {
      ...
      showToast(fieldDeleteKindLabel(entry) + '已删除', 'success');
    } catch (error: unknown) {
  ...
  function importFields(event: Event): void {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        showToast('导入组失败: ' + (error as Error).message, 'error');
        input.value = '';
        return;
      }
      ...
      try {
        ...
        showToast('已导入 ' + entries.length + ' 个组', 'success');
      } catch (error: unknown) {
        showToast('导入组失败: ' + (error as Error).message, 'error');
        await loadFields();
      } finally {
  ...
  async function confirmSimplePickerSelection(): Promise<void> {
    ...
    try {
      ...
      analysis.setFieldGroupParams(name, { scale: 1, offset: 0, scaleInput: '1', offsetInput: '0' });
      await analysis.addCurveSelection(curves, name);
      simplePicker.value.open = false;
      showToast('组已保存', 'success');
    } catch (error: unknown) {
