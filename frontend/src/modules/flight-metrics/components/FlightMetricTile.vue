...
import { barPct } from '@/modules/flight-metrics/utils/flight-fields';
...
const motorMeta = computed(() => three.value.telemetry.meta?.motor || '');
...
const modeName = computed(() => analysis.translateMode(analysis.currentFlightMode()) || '—');
const modeColor = computed(() => analysis.flightModeColor(analysis.currentFlightMode()));
...
<template>
  <div class="metric-tile" :class="tileClass" :style="modeStyle">
    ...
    <template v-else-if="kindMotor">
      <span class="metric-motors-head">{{ item.name }}<small>{{ motorMeta }}</small></span>
      <div class="motor-bars">
        <div class="motor-bar" v-for="m in motorValues" :key="'mot-' + m.label" :title="'C' + m.label + ': ' + scene.formatPwm(m.value)">
          <div class="motor-bar-track">
            <div class="motor-bar-fill" :class="{ saturated: scene.motorSaturated(m.value) }" :style="{ height: scene.motorBarPct(m.value) + '%' }"></div>
          </div>
    ...
    <template v-else-if="item.kind === 'field' && item.render === 'number'">
      <button class="metric-main" type="button" @click="openEditor"><span>{{ item.name }}</span></button>
      <strong>{{ scene.formatMetric(mulValue(singleValue), unit) }}</strong>
    </template>
    ...
    <template v-else-if="item.kind === 'group' && item.render === 'number'">
      <button class="metric-main metric-group-head" type="button" @click="openEditor"><span>{{ item.name }}</span></button>
      <div class="metric-group-channels">
        <div class="metric-channel" v-for="c in groupValues" :key="item.id + '-' + c.key">
          ...
          <strong>{{ scene.formatMetric(mulGroupValue(c), unitFor(c.key)) }}</strong>
        </div>
      </div>
    </template>
