...
export function createFieldResolution(state: Ref<MetricState>) {
  ...
  function fieldValue(typeField: string): number | null {
    ...
    switch (typeField) {
      case SPEED_FIELD_KEY:
        return finiteOrNull(scene.three.current.speed);
      case VSPEED_FIELD_KEY:
        return finiteOrNull(scene.three.current.verticalSpeed);
      case VOLT_FIELD_KEY:
        return finiteOrNull(scene.currentVoltage());
      default:
        break;
    }
    const parts = splitTypeField(typeField);
    if (!parts) return null;
    const t = scene.three.playback.timeMs;
  ...
  function fieldValues(typeField: string): LabeledValue[] {
    if (typeField === MOTOR_FIELD_KEY) {
      return useScene3dStore().currentMotors() as LabeledValue[];
    }
