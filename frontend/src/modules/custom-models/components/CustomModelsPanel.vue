...
watch(tab, () => mapLibreStore.setGizmoActive(false));
</script>
...
<template>
  <div class="custom-models-panel">
    <div class="cm-head">
      <strong>自定义模型</strong>
      <button class="btn-icon" type="button" @click="mapLibreStore.closeCustomModelsPanel()" title="关闭，恢复姿态面板">
        <AppIcon name="close" :size="15" />
      </button>
    </div>
    <div class="cm-tabs">
      <button class="btn btn-xs" :class="{ active: tab === 'single' }" @click="tab = 'single'">单个模型</button>
      <button class="btn btn-xs" :class="{ active: tab === 'group' }" @click="tab = 'group'">模型组</button>
    </div>
